...
In this document, we run the rational framework on effect size judgement experiment in Kale et al. We first use two statistical models to predict behavioral agent's probability of superiority (PoS) distribution and decision distribution. We use the models from Kale et al. to generate.

In effect size judgement experiment, we consider two kinds of behavioral agents. The first kind reports their belief of PoS, and the second kind directly reportsthe decision (whether to hire new player in the setting of Kale et al.). The framework basically is a function that estimates five parameters:
...
We generate the data generate the behavioral agents' belief of PoS and decisions by the posterior predictive distribution of the models.
...
These are constants and data initialization.
...
We then define the utility functions need by rational agent calculation.
