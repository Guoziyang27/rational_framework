...
The framework basically is a function that estimates four parameters:
...
3.  The *behavioral score* ($B$): the expected score of the behavioral agent predicted by generative statistic model.
...
meta.df = df %>% 
  dplyr::select(scenario, vis, trial_normalized, distribution, mu, sigma, nu, tau)
...
scoring_rule = function(rider_at_stop, distribution, scenario) {
  ...
  payoff <- function(rider_at_stop, bus_at_stop, second_bus_at_stop, wait_penalty,
          ...
          arrived_before_bus <- as.integer(rider_at_stop <= bus_at_stop - 1)
          arrived_after_bus <- as.integer(rider_at_stop > bus_at_stop - 1)
  ...
  pointwise.payoff <- lapply(0:(30 * 30 - 1),
                             function(m,
                                      wait_penalty,
                                      delay_bonus,
                                      destination_reward,
                               ...
                               first_bus = (m %/% 30) + 1
                               second_bus = (m %% 30) + 1
                            first_prob <- distribution[first_bus]
                            second_prob <- distribution[second_bus]
...
get_rational_posterior = function(sample_df, distribution_df) {
  ...
  for (scenario_id in scenario_ids) {
    ...
    for (vis_name in vis_names) {
      ...
      value_seq = list()
      for (value in values) {
        distributions = case_data[case_data[vis_name] == value, ] |> dplyr::select("discrete_1":"discrete_30") |> data.matrix()
        
        distribution = colSums(distributions) / nrow(distributions)
        payoff = Reduce("max", lapply(1:30, 
...
get_behavioral = function(sample_df, distribution_df) {
  # behavioral_action_payoff = array(c(), dim=nrow(sample_df))
  # for (i in 1:nrow(sample_df)) {
  #   behavioral_action_payoff = append(behavioral_action_payoff, 
  #                                payoff_mapping[[as.character(sample_df[i, "scenario"][[1]]) ]]
  #                                 [[ sample_df[i, "distribution"][[1]] ]]
  #                                 [[ sample_df[i, "pred_response"][[1]] ]])
  # }
  sample_df |>
    ungroup() |>
    mutate(scenario = as.character(scenario)) |> 
    rowwise() |> 
    mutate(behavioral_payoff = payoff_mapping[[scenario]][[distribution]][[pred_response]]) |>
...
c(all_behavioral, all_calibrated_behavioral)
```
