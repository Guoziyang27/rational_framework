...
distribution_df = df %>%
  ...
  unique() %>%
  rowwise() %>%
  # mutate(discrete = list((pBCT((-15:30) + 0.5 + 15, mu, sigma, nu, tau) -
  #                           pBCT((-15:30) - 0.5 + 15,  mu, sigma, nu, tau)) /
  #                          (pBCT(30 + 15, mu, sigma, nu, tau) - pBCT(0, mu, sigma, nu, tau)))) %>%
...
pred.df$pred_response = round(pmax(pmin(pred.df$pred_response, 30), 0))
  ...
  for (i in 1:nrow(sample_df)) {
    behavioral_action_payoff = append(behavioral_action_payoff, 
                                 payoff_mapping[[as.character(sample_df[i, "scenario"][[1]]) ]]
                                  ...
                                  [[ sample_df[i, "pred_response"][[1]] + 1 ]])
  }
  sample_df |>
    group_by() |>
    mutate(behavioral_payoff = behavioral_action_payoff) |>
...
get_calibrated_behavioral = function(sample_df, distribution_df) {
  ...
  for (scenario_id in scenario_ids) {
    for (vis_id in vis_ids) {
      case_data = sample_with_distribution |> filter(scenario == scenario_id,
                                                     vis == vis_id)
      responses = unique(sample_with_distribution$pred_response)
...
all_rational_posterior = get_rational_posterior(df, distribution_df)
```


```{r calculation rational prior}
all_rational_prior = get_rational_prior(df, distribution_df)
```
