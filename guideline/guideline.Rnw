...
\section{General guide for applying the rational agent framework}

\subsection{Identifying the decision problem}
...
\begin{itemize}
    ...
    \item \textbf{data generating model}: the distribution of the distributions that states are generated from. In the weather forecast example, the data generating model is a uniform distribution on $\{Normal(5, 2), Normal(5, 3), Normal(5, 4), Normal(5, 5)\}$.
    ...
    \item \textbf{signals}: the visualizations you show to users to aid their decisions.
    \item a \textbf{payoff} or \textbf{scoring rule}: the payoff function used in experiment to incentivize/score users' behaviors. In general, you can think of the scoring rule in the following two forms.
    \begin{itemize}
    \item payoff: $\actionspace\times\statespace\to \mathbb{R}$, a function mapping an action and the state to a real-valued payoff. e.g. how many points you lose if you salt when it doesn't freeze or don't salt but it freezes.
    \item proper score: $\distover{\statespace}\times\statespace\to \mathbb{R}$ a function mapping the probability distribution over state and the state to a real-valued payoff. The proper score applies the optimal action given the belief (a probability distribution), and is a proper measure of the quality of a decision. See Gneiting and Raftery, "Strictly Proper Scoring Rules, Prediction, and Estimation" (2007) for more information.
    \end{itemize}
...
\item to evaluate the effect of seeing a visualization in an experiment.
\end{itemize}
...
\begin{itemize}
\item if it is believed that behavioral agents are incentivized by monetary rewards, the experimenter should pre-experimentally calculate the value of information in real money, and evaluate the incentive.
...
\subsection{Pre-experiment analysis}

In this section, we will show how to calculate the three pre-experiment quantities (rational baseline, rational benchmark, and value of information) that the rational agent framework offers. The rational agent's prior belief is known before seeing the visualization, and represents the unconditional probability that each state is drawn according to the data generating model. Given a visualization signal, the rational agent will update their beliefs about the state based on the joint distribution between visualizations and states. We provide the pseudocode in Alg. \ref{alg:baseline} and Alg. \ref{alg:benchmark} to calculate the pre-experiment quantities. 

For the rational baseline, we assume that the rational agent has no access to the signal, i.e. responds based on only the prior belief. Therefore, when simulating this agent on real experiment data (Alg. \ref{alg:baseline}), the agent won't care about the visualization or any provided information but always take the same action based on the prior belief.

\begin{algorithm}[!h]
\caption{Rational baseline}\label{alg:baseline}
\KwIn{the experimental data $D$ with each row representing one experimental trial, prior beliefs about the state $p(d)$, the action space $A$, and the scoring rule $S$}
\KwOut{the rational baseline $\rprior$}
$belief \gets E_{d\sim p(d)}(d)$\;\Comment{the prior belief about states}
$action \gets \argmax_{a \sim A}{E_{\theta \sim belief}(S(a, \theta))}$\;\Comment{the action made on prior belief}
$payoff \gets 0$\;
\For{$row \in D$}{
  $dist \gets \text{the actual distribution that states are drawn used in } row$\;
  $payoff \gets payoff + E_{\theta \sim dist}(S(action, \theta))$\;
}
$\rprior \gets payoff / \text{\# of row in }D$\;\Comment{rational baseline is the rational agent's expected payoff in the experiment under only their prior belief}
\end{algorithm}

For the rational benchmark, we assume the rational agent has access to the visualization (signal). After seeing the visualization (signals), the rational agent will update their belief about the state based on the joint distribution between visualizations and states. For example, the distribution the visualization signals $v$ on the distribution of states $d$ is $p(v|d)$ and the prior belief about the states, i.e. the data generating model, is $p(d)$. Then the posterior belief about the state could be $p(d|v) = p(v|d) \cdot p(d) / p(v)$. See the detail algorithm in Alg. \ref{alg:benchmark}.

\begin{algorithm}[!h]
\caption{Rational benchmark}\label{alg:benchmark}
\KwIn{the experimental data $D$ with each row representing one experimental trial, prior beliefs about the state $p(d)$, the action space $A$, and the scoring rule $S$}
\KwOut{the rational benchmark $\rscore$}
$payoff \gets 0$\;
\For{$row \in D$}{
  $v \gets \text{visualization used in } row$\; 
  $p(d|v) = p(v|d) \cdot p(d) / p(v)$\;\Comment{the posterior belief about the distribution of states}
  $belief \gets E_{d\sim p(d|v)}(d)$\;\Comment{the posterior belief about states}
  $action \gets \argmax_{a \sim A}{E_{\theta \sim belief}(S(a, \theta))}$\;\Comment{the action made on posterior belief}
  $dist \gets \text{the actual distribution of states used in } row$\;
  $payoff \gets payoff + E_{\theta \sim dist}(S(action, \theta))$\;\Comment{the expected payoff with posterior information}
}
$\rscore \gets payoff / \text{\# of row in }D$\;
\end{algorithm}
...
The rational agent framework also offers two post-experiment measures (behavioral score and calibrated behavioral score) to quantify behavioral performance. The behavioral score $\bscore$ is the payoff that the behavioral agents (i.e. real users) get or are expected to get in the experiment, which is always below the rational benchmark $\rscore$ and can be either above or below the rational baseline $\rprior$. Importantly, if the behavioral score is below the rational baseline, then from the scores alone we cannot reject the hypothesis that the behavioral agent got no useful information from the visualization, because even with no information, the rational agent performs better. Therefore, we propose the calibrated behavioral score, where you can use the rational agent to calibrate the behavioral score to fall within the interval between the rational baseline and the rational benchmark, to understand how much information the behavioral agents didn't get.

You can follow Alg. \ref{alg:behavioral} to estimate the payoff of behavioral agents' actions.
...
For the calibrated behavioral score, you simulate the rational agent as in calculating the rational baseline and the rational benchmark, but the rational agent is not using the visualizations as signals but rather using the actions made by behavioral agents. This means that when seeing a behavioral agent's action $a$, the rational agent will update the belief about states based on the joint distribution between behavioral actions and states, i.e. $p(d|a) = p(a|d) \cdot p(d) / p(a)$ (Alg. \ref{alg:calibrated}).

\begin{algorithm}[!h]
\caption{Calibrated behavioral score}\label{alg:calibrated}
\KwIn{the experimental data $D$ with each row representing one experimental trial, prior beliefs about state $p(d)$, the action space $A$, and the scoring rule $S$}
\KwOut{the calibrated behavioral score $benchmark$}
$payoff \gets 0$\;
\For{$row \in D$}{
  $signal \gets \text{action made by the behavioral agent in } row$
  $p(d|signal) = p(signal|d) \cdot p(d) / p(signal)$\; \Comment{the posterior belief of the distribution of states in calibration}
  $belief \gets E_{d\sim p(d|signal)}(d)$\;\Comment{the belief of the states}
  $action \gets \argmax_{a \sim A}{E_{\theta \sim belief}(S(a, \theta))}$\; \Comment{the action made on posterior belief}
  $dist \gets \text{the actual distribution of states used in } row$\;
  $payoff \gets payoff + E_{\theta \sim dist}(S(action, \theta))$\; \Comment{the expected payoff with posterior information}
}
...
\section{Walkthrough example: weather forecast visualization}

\subsection{Setting up for R code}
...
\end{itemize}
%The first is the binary decision on whether to salt the parking lot and the second is their belief about the possibility of freezing in discrete levels after view a visualization about daily lowest temperature. 
...
\begin{itemize}
    ...
    \item Assume daily low temperature $t \sim N(\mu, \sigma^2)$; $\Pr[\theta = 1] = \Pr[t \leq 0]$; $\mu = 5$ fixed and $\sigma$ uniformly from $\{2, 3, 4, 5\}$. The information provided by different visualization strategies:
    \begin{itemize}
    ...
    \item scoring rule: 
    \begin{itemize}
    \item payoff:  $S_{decision}(a, \theta)$ for reporting the decision of salting, see equation 2 in the paper.
    \item proper score: $S_{belief}(b, \theta)$ for reporting the belief of freezing, see eq. \ref{eq:belief_scoring_rule} below.
    \end{itemize}
\end{itemize}

\begin{equation}
S_{belief}(b, \theta) = S_{decision}(\argmax_{a \in A}{S_{decision}(a, b)}, \theta)
\label{eq:belief_scoring_rule}
\end{equation}
...
\subsection{For the purposes of this walkthrough only: Simulate behavioral data using quantal response equilibrium}
...
\subsubsection{Explicit Calculations for Weather Forecasting Experiment}
...
\subsubsection{Rational benchmark}
...
When seeing "mean" visualization, the rational agent didn't get the information about sigma, so they can't update the belief, and the rational benchmark for both reporting action and belief turns out to be $-7.96$, which is the same as rational baseline. For the other visualization, the rational benchmark for both reporting action and belief is $-5.69$.
...
\subsubsection{Behavioral score}
...
Some results of the behavioral agents ($-8.47$, $-8.37$, $-8.59$, and $-8.33$) are below the rational baseline ($-7.96$). Even though these results (in this example) come from some hypothetical behavioral agents who are simulated by quantal response equilibrium theory, this situation could also happen in a real experiment setting, where the behavioral agents might not follow a rational updating or decision rule or don’t have a proper prior belief. Solely comparing these raw behavioral scores with the rational baseline and benchmark will entangle the loss cased by the information in visualization and the loss cased by the behavioral agents’ improper prior belief or decision rule. Therefore, we propose the calibrated behavioral score, which can calibrate the behavioral agent by a Bayesian updating process and separate these two losses.

\subsubsection{Calibrated behavioral score}

We calibrate the behavioral agent by using a rational agent accessing their decisions as signals.
