...
\item The experimenter can also use a proper scoring rule solely for the purpose of evaluation. The rational baseline, benchmark and value of information can be calculated pre-experimentally to quantify the ``room for improvement'' in the experiment, thus to quantify information asymmetry. 
\end{itemize}
