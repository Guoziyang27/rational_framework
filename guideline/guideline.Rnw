...
\begin{itemize}
    ...
    \item \textbf{data generating model}: the process that generates the data shown in visualizations, from which the distribution of states can be inferred. In the weather forecast example, the data generating model is drawn uniformly from $\{Normal(5, 2), Normal(5, 3), Normal(5, 4), Normal(5, 5)\}$.
...
%    the payoff evaluates the quality score or payoff that the experiment assigns to participants depending on their action and the state in reality,  For each payoff function, we can construct a proper scoring rule to evaluate the 
    \end{itemize}
...
\begin{itemize}
\item if it is believed that behavioral agents are incentivized by monetary rewards, the experimenter should pre-experimentally calculate the value of information in real money, and evaluate the incentive. The experimenter might prefer to choose a scoring rule with higher incentives compared to a base payment. [JH - not sure what previous sentence means]
\item The experimenter can also use a proper scoring rule solely for the purpose of evaluation. The rational baseline, benchmark and value of information can be calculated pre-experimentally to compare with behavioral performance.  [JH - I think we should try to say why one would do an analysis with a proper scoring rule? is it bc you assume people will be well incentivized (even if they aren't paid) and you want to ensure you have a big enough gap between baseline and benchmark? Could add a sentence like 'For example, if an experiment uses a flat rule, or relies on volunteers, under an assumption that the participants will be naturally incentivized, they can use a proper scoring rule to analyze the design.  ]
\end{itemize}
...
$payoff \gets 0$\;
\For{$row \in D$}{
  $dist \gets \text{actual data generating model used in } row$\;
  $payoff \gets payoff + E_{\theta \sim dist}(S(action, \theta))$\;
}
[JH - I didn't change some references to data generating model above (and below) but we need to]
...
For the rational benchmark, we assume the rational agent has access to the visualization (signal). After seeing the visualization (signals), the rational agent will update their belief about the state based on the joint distribution between visualizations and states. For example, the visualization signal $v$ provides information about the possibility that this visualization could occur in data generating models $d$ as $p(v|d)$ and the prior belief about the data generating model is $p(d)$. Then the posterior belief about the data generating models could be $p(d|v) = p(v|d) \cdot p(d) / p(v)$. See the detail algorithm in Alg. \ref{alg:benchmark}.

\begin{algorithm}[!h]
\caption{Rational benchmark}\label{alg:benchmark}
\KwIn{the experimental data $D$ with each row representing one experimental trial, prior belief about data generating model $p(d)$, the action space $A$, and the scoring rule $S$}
\KwOut{the rational benchmark $\rscore$}
$payoff \gets 0$\;
\For{$row \in D$}{
  $v \gets \text{visualization used in } row$\; 
  $p(d|v) = p(v|d) \cdot p(d) / p(v)$\;\Comment{the posterior belief about data generating models}
